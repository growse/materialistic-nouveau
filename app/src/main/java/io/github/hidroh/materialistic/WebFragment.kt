...
public class WebFragment extends LazyLoadFragment
    ...
    public static final String EXTRA_ITEM = WebFragment::class.java.getName() +".EXTRA_ITEM"
    private static final String STATE_EMPTY = "state:empty"
    private static final String STATE_READABILITY = "state:readability"
    static final String ACTION_FULLSCREEN = WebFragment::class.java.getName() + ".ACTION_FULLSCREEN"
    static final String EXTRA_FULLSCREEN = WebFragment::class.java.getName() + ".EXTRA_FULLSCREEN"
    ...
    private final Preferences.Observable mPreferenceObservable = Preferences.Observable()
    ...
    public void onAttach(Context context) {
        ...
        LocalBroadcastManager.getInstance(context).registerReceiver(mReceiver,
                IntentFilter(ACTION_FULLSCREEN))
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        if (isNewInstance()) {
            mScrollableHelper = KeyDelegate.NestedScrollViewHelper(mScrollView)
            mSystemUiHelper = AppUtils.SystemUiHelper(getActivity().getWindow())
    ...
    private void reloadUrl(String url, @Nullable String pdfFilePath) {
        ...
        if (pdfFilePath != null && TextUtils.equals(PDF_LOADER_URL, url)) {
            setProgress(80)
            mIsPdf = true
            mPdfAndroidJavascriptBridge = PdfAndroidJavascriptBridge(pdfFilePath, PdfAndroidJavascriptBridge.Callbacks() {
    ...
    private void parse() {
        ...
        mReadabilityClient.parse(mItem.getId(), mItem.getUrl(), ReadabilityCallback(this))
    }
    ...
    private void bindContent() {
        if (mItem is Item) {
            ...
            mItemManager.getItem(mItem.getId(), ItemManager.MODE_DEFAULT, ItemResponseListener(this))
        }
    }
    ...
    private void setUpWebControls(View view) {
        ...
        view.findViewById(R.id.button_exit).setOnClickListener(v ->
                LocalBroadcastManager.getInstance(getActivity()).sendBroadcast(
                        Intent(WebFragment.ACTION_FULLSCREEN)
    ...
    private void setUpWebView(View view) {
        ...
        mWebView.setWebViewClient(AdBlockWebViewClient(Preferences.adBlockEnabled(getActivity())) {
    ...
    private void downloadFileAndRenderPdf() {
        mFileDownloader.downloadFile(mItem.getUrl(), PDF_MIME_TYPE, FileDownloader.FileDownloaderCallback() {
    ...
    open class ReadabilityCallback : ReadabilityClient.Callback {
        private var mReadabilityFragment: WeakReference<WebFragment>? = null

        @Synthetic
        constructor(webFragment: WebFragment) {
            mReadabilityFragment = WeakReference<>(webFragment)
        }
    ...
    open class ItemResponseListener : ResponseListener<Item> {
        private var mFragment: WeakReference<WebFragment>? = null

        @Synthetic
        constructor(webFragment: WebFragment) {
            mFragment = WeakReference<>(webFragment)
        }
